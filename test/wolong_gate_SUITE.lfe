...
(defun status_parser_valid_record (_config)
  (let* ((result (wolong-status:parse
...
(defun status_parser_numeric_exit_code (_config)
  (let* ((result (wolong-status:parse #b("PANDAPI_STATUS\tstatus=input_invalid\texit_code=22\n")))
         (fields (element 2 result)))
    (equal 22 (map-get fields 'exit-code))))

(defun status_parser_multiline_uses_final_record (_config)
  (let* ((stderr #b("PANDAPI_STATUS\tstatus=input_invalid\texit_code=22\nnoise\nPANDAPI_STATUS\tstatus=ok\texit_code=0\n"))
...
(defun mapper_covers_managed_status_vocabulary (_config)
  (let ((output-path (temp-path "wolong-gate-mapper.plan")))
    ...
    (assert-mapped 'parser 10 #b("cli_usage_error") 'error 'cli-usage-error output-path)
    (assert-mapped 'parser 20 #b("input_unavailable") 'error 'input-unavailable output-path)
    (assert-mapped 'parser 21 #b("output_unavailable") 'error 'output-unavailable output-path)
    (assert-mapped 'parser 22 #b("input_invalid") 'error 'input-invalid output-path)
    (assert-mapped 'parser 30 #b("unsupported_feature") 'error 'unsupported-feature output-path)
    (assert-mapped 'parser 31 #b("legacy_surface") 'error 'legacy-surface output-path)
    (assert-mapped 'parser 32 #b("experimental_surface") 'error 'experimental-surface output-path)
    (assert-mapped 'parser 33 #b("future_surface") 'error 'future-surface output-path)
    (assert-mapped 'parser 40 #b("timeout") 'error 'managed-timeout output-path)
    (assert-mapped 'parser 41 #b("resource_limit") 'error 'resource-limit output-path)
    (assert-mapped 'parser 42 #b("interrupted") 'error 'interrupted output-path)
    (assert-mapped 'parser 50 #b("dependency_failure") 'error 'dependency-failure output-path)
    (assert-mapped 'parser 51 #b("child_process_failure") 'error 'child-process-failure output-path)
    (assert-mapped 'parser 60 #b("internal_error") 'error 'internal-error output-path)
    (assert-mapped 'parser 99 #b("new_status") 'error 'unmapped-status output-path)
    (assert-classified 'parser (tuple 'ok (completed-result 0 #b("diagnostic only\n")))
...
(defun mapper_exec_error_is_typed (_config)
  (let* ((result (wolong-gate:classify
                  'parser
...
(defun engine_domain_no_plan_distinct_from_failures (_config)
  (let ((output-path (temp-path "wolong-engine-missing-artifact.plan")))
    (assert-mapped 'engine 22 #b("input_invalid") 'error 'input-invalid output-path)
...
(defun supervised_parse_ground_solve_fixture (_config)
  ...
  (let* ((config (ok-value (wolong-config:validate)))
         ...
         (parsed (wolong-gate:run-parser parser
                                         (fixture-path "gate-contract-substrate/minimal/domain.hddl")
...
(defun temp-workdir ()
  (filename:join (list "/tmp" "wolong-gate-suite")))
...
(defun assert-mapped (gate exit-status status expected-top expected-reason output-path)
...
(defun assert-classified (gate runner-result expected-top expected-reason output-path)
  (let ((result (wolong-gate:classify gate runner-result output-path)))
    (equal expected-top (element 1 result))
    (case expected-top
      ('ok 'ok)
      ('domain-no-plan 'ok)
      ('error (equal expected-reason (element 1 (element 2 result)))))))
