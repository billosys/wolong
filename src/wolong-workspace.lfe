  ...
  ((base-dir attempts)
   (let* ((dispatch-id (dispatch-id))
     ...
     (case (file:make_dir dispatch-dir)
       ('ok `#(ok ,(workspace base-dir dispatch-id dispatch-dir)))
       (`#(error ,reason) (handle-dispatch-error base-dir dispatch-dir attempts reason))))))
...
(defun base-unavailable (base-dir reason)
  `#(error #(base-unavailable
             ,(map 'base-dir base-dir 'reason reason))))
...
(defun workspace (base-dir dispatch-id dispatch-dir)
  (map 'base-dir base-dir
       ...
       'path dispatch-dir
       'artifacts (map 'parser (filename:join dispatch-dir "parser.htn")
